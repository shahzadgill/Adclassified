<?php
	class Database{
		...
		function __construct(){
			...
			$dbname = 'adclassified';
			/*
			$servername = 'mysql1107.ixwebhosting.com';
			$username = 'BBBm8yr_shahzad';
			$password = 'Akvon555';
			$dbname = 'BBBm8yr_shahzad_db1';*/
			$this->dbh = new mysqli($servername, $username, $password, $dbname);
				if ($this->dbh->connect_error) {
	    		die("Connection failed: " . $this->dbh->connect_error);
			} 
			/*$auth = new auth();
	    	$this->uid = $auth->get_id();*/
		}
		...
		function mysql_get_row($table,$columns,$where){
			//die('select '.$columns.' from '.$table.' where '.$where);
	...
	foreach($_REQUEST as $key=>$value){
		if(is_string($value))
			$_REQUEST[$key]=$db->escape_string($value);
	}

	//$GLOBALS['uid'] = $auth->get_id();
